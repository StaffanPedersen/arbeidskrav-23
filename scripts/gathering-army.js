...
const showAllWarriors = () => {
  ...
  warriors.forEach((warrior) => {
    htmltext += `
        ...
        <article class="rounded-lg mx-auto shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]" >
        ...
        <p class="text-center"> Gold: ${warrior.priceGold}  </p>
  ...
  warmachines.forEach((warrior) => {
    htmltext += `
        ...
        <article class="rounded-lg shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
        <h3 class="mb-2 text-center text-2xl font-bold tracking-tight text-gray-900 dark:text-white">${
          warrior.name
        }</h3>
        <img src="images/${warrior.img}" alt="${warrior.name}" />
        <p class="text-center"> Gold: ${warrior.priceGold} Wood: ${
      warrior.priceWood ? ` ${warrior.priceWood}` : "0"
    } Metal: ${warrior.priceMetal ? ` ${warrior.priceMetal}` : "0"} </p>
